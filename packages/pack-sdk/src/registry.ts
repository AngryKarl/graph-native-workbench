...
async function fetchBytes(
  ...
  if (declared > limit) {
    await response.body?.cancel('Registry response exceeded its byte limit.');
    throw new Error(`Registry response exceeds the ${limit} byte limit.`);
  }
  ...
  try {
    while (true) {
      ...
      if (total > limit) {
        await reader.cancel('Registry response exceeded its byte limit.');
        throw new Error(`Registry response exceeds the ${limit} byte limit.`);
      }
      chunks.push(value);
    }
