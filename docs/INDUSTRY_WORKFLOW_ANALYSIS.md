...
| Quantitative finance | Research subgraphs, dynamic instrument Map, risk/compliance gates, execution events and reconciliation | Pass; fourth standard Pack implemented | Market data, backtest engine, OMS/EMS, broker and books-and-records system |
| Healthcare diagnostics | Identity-attributed work, consent gate, parallel analysis, specialist approval, FHIR-shaped context and follow-up | Pass; fifth standard non-clinical Pack implemented | EHR/FHIR server, PACS/LIS, scheduling, clinical decision authority and regulated identity |
| Robotics and fleet operations | Task events, bid Map, bounded replanning, resource decisions, telemetry correlation, escalation and maintenance | Pass; sixth standard coordination Pack implemented | ROS 2/OpenRMF, real-time control, safety controller, resource locks and telemetry bus |
...
4. **Quantitative Finance Pack** — implemented with governed research, risk,
   execution and reconciliation fixtures.
5. **Healthcare Diagnostics Pack** — implemented with consent-aware request,
   advisory analysis, specialist review and follow-up fixtures.
6. **Robotics and Fleet Operations Pack** — implemented with allocation,
   dispatch, observation, bounded replanning and maintenance fixtures.
