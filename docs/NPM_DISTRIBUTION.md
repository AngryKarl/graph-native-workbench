...
```bash
pnpm dlx /absolute/path/to/release/npm/graphwork-0.2.2.tgz --version
pnpm dlx /absolute/path/to/release/npm/graphwork-0.2.2.tgz demo
```
...
`latest`. `graphwork@0.2.1` is the interaction-polish patch. `graphwork@0.2.2`
is the current Graphwork identity and security-hardening patch under `latest`.
