...
describe('Registry release publisher', () => {
  it('builds the reference Packs into a signable HTTPS catalog', async () => {
    ...
    const release = await buildPackRegistryRelease(
      JSON.parse(await readFile(configPath, 'utf8')) as unknown,
      {
        ...
        outputDirectory,
        artifactBaseUrl: 'https://github.com/example/project/releases/download/packs-v0.4.0',
        expiresInDays: 30,
        now,
      },
    );
    ...
    expect(release.payload).toMatchObject({
      ...
      packs: [
        { id: 'robotics_fleet', version: '0.4.0', engineRange: '^0.4.0' },
        { id: 'healthcare_diagnostics', version: '0.4.0', engineRange: '^0.4.0' },
        { id: 'quantitative_finance', version: '0.4.0', engineRange: '^0.4.0' },
        { id: 'cybersecurity_response', version: '0.4.0', engineRange: '^0.4.0' },
        { id: 'data_mlops', version: '0.4.0', engineRange: '^0.4.0' },
        { id: 'software_delivery', version: '0.4.0', engineRange: '^0.4.0' },
      ],
    });
    ...
    expect(release.payload.packs[0]?.artifact).toBe(
      'https://github.com/example/project/releases/download/packs-v0.4.0/packs/robotics_fleet-0.4.0.gpack',
    );
  ...
  it('rejects insecure publishing URLs and excessive catalog lifetimes', async () => {
    ...
    await expect(buildPackRegistryRelease({
      ...
      artifactBaseUrl: 'https://example.com/releases/',
      expiresInDays: 30,
    })).rejects.toThrow(/duplicate Pack "research@0.4.0"/);
  });
});
